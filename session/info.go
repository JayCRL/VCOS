...
func BuildRuntimeInfoResult(sessionID, query, cwd string, svc *Service) (protocol.RuntimeInfoResultEvent, error) {
	...
	case "doctor":
		...
		ghPath, ghErr := exec.LookPath("gh")
			adbPath := DefaultHostInfoProvider.ADBPath()
			emulatorPath := DefaultHostInfoProvider.EmulatorPath()
			adbAvailable := DefaultHostInfoProvider.ADBAvailable()
			emulatorAvailable := DefaultHostInfoProvider.EmulatorAvailable()
			adbMessage := DefaultHostInfoProvider.ADBMessage()
		items := []protocol.RuntimeInfoItem{
			...
			{Label: "adb_cli", Value: adbPath, Available: adbAvailable, Status: availabilityStatus(adbAvailable), Detail: adbMessage},
			{Label: "emulator_cli", Value: emulatorPath, Available: emulatorAvailable, Status: availabilityStatus(emulatorAvailable), Detail: adbMessage},
